...
#ifdef CC13XX_CC26XX_WRITE_LIMIT
s32_t SPIFFS_write_delegate(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
#else
...
#ifdef CC13XX_CC26XX_WRITE_LIMIT
/*
 * Flash memory on CC13XX and CC26XX devices is divided into rows of 128 or 256
 * bytes of length (refer to datasheet for exact size).  These rows can support
 * up to 83 write operations before suffering effects of row disturb in which
 * data can be corrupted.  While SPIFFS writes to flash efficiently, there
 * are still a few situations where the 83 write limit can be violated:
 *
 *   * Setting logical page size very small or setting the logical block size
 *     too large can lead to many logical pages in a logical block.  Updating
 *     pages may cause more than 83 write operations on a logical block's index
 *     page (first page in the logical block).
 *
 *   * Continuously writing (& flushing to flash) a single byte of data to the
 *     end of a file.
 *
 * The following conditions must be followed to prevent exceeding the 83
 * write limit:
 *
 ...
 *
 *   * Limit write operations to be 4 bytes or greater in size
 *     (256 bytes per page / 83 write operations = 3.08 ~ 4 bytes).
 *
 ...
 * verify and return an error if the amount of logical pages in a logical block
 * is > 32.
 *
 * The second condition may be a little harder to meet as some applications
 * may depend on writing a single byte at a time.  To satisfy the 83 write
 * limit while still allowing < 4 byte writes to a file, the SPIFFS_write()
 * function has been modified to do the following:
 *
 *   * Perform a normal write operation if the file is empty or the amount of
 *     data to be written is > 4 bytes.
 *
 *   * Otherwise, copy the current page to a new page and then write the data to
 *     the new page.  By copying the content to a new page we restart the 83
 *     write limit on the new page.
 *
 * @note Writing < 4 bytes with the aforementioned change will increase
 *       the speed at which memory is used and will cause garbage collection
 *       to occur more frequently.  Writes < 4 bytes will also take longer to
 *       complete due to relocating the current page.  Most efficient behavior
 *       is achieved when write operations are 4 bytes or greater.
 *
 * The aforementioned conditions/changes are enabled when source is compiled
 * with "CC13XX_CC26XX_WRITE_LIMIT=1" defined in the compiler options.
 *
 * @note Other SPIFFS APIs (SPIFFS_open(), SPIFFS_read(), etc.) are unaffected.
 *
 */
s32_t SPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  SPIFFS_API_DBG("%s "_SPIPRIfd " "_SPIPRIi "\n", __func__, fh, len);
