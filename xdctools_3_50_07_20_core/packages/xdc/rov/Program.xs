...
function addCMod(cmod)
{
    ...
    if (cmod.viewMap.length > 0) {
        for (var i = 0; i < cmod.viewMap.length; i++) {
            ...
            //for (var p in cmod.capsule[cmod.viewMap[i].structName]) {
            //    status[p] = null;
            //}
            //cmod.capsule[cmod.viewMap[i].structName].prototype.$status = status;
        }
    }
...
/*
 *  @(#) xdc.rov; 1, 0, 1,0; 5-22-2018 17:33:19; /db/ztree/library/trees/xdc/xdc-E05/src/packages/
 */
